...
def upload_to_storage(file_data, storage_key):
    s3.put_object(Bucket=settings.MINIO_BUCKET, Key=storage_key, Body=file_data)
...
def move_to_trash(source_key, trash_key):
    ...
    s3.delete_object(Bucket=settings.MINIO_BUCKET, Key=source_key)
...
def restore_from_trash(trash_key, original_key):
    ...
    s3.delete_object(Bucket=settings.MINIO_BUCKET, Key=trash_key)
...
def set_trash_lifecycle_policy():
    ...
    s3.put_bucket_lifecycle_configuration(
        Bucket=settings.MINIO_BUCKET, LifecycleConfiguration=lifecycle_config
    )
